...
// LLM provider API key — docelowo przenieść do backend proxy (Supabase)
export const LLM_PROVIDER_API_KEY = process.env.EXPO_PUBLIC_LLM_PROVIDER_API_KEY ?? '';
...
const ANTHROPIC_API_URL = 'https://api.llm-provider.com/v1/messages';
...
export async function sendDJChatMessage(
  ...
  try {
    ...
    if (!response.ok) {
      ...
      console.log('LLM Chat API error:', response.status, errorBody);
      const fallback = getDJFallbackResponse(personaId);
      logChatMessage(personaId, 'assistant', fallback);
      return fallback;
    }
    ...
    // Wyciąg tekst z odpowiedzi — LLM może zwrócić mieszankę text + web_search_tool_result
    ...
    console.log('LLM Chat API error:', error);
    const fallback = getDJFallbackResponse(personaId);
    logChatMessage(personaId, 'assistant', fallback);
    return fallback;
  }
